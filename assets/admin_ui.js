// ── Admin UI Overrides ──────────────────────────────────────────────────────
// Runs after admin_logic.js — overrides render functions to match new design.
// All API calls use EXACT action names from api/admin.php
...
var _adminNameEl = document.getElementById('adminName');
if (_adminNameEl) new MutationObserver(_syncTopbar).observe(_adminNameEl, {childList:true,subtree:true,characterData:true});
_syncTopbar();

// ── Active nav ────────────────────────────────────────────────────────────────
(function() {
  var orig = (typeof showTab === 'function') ? showTab : null;
  if (!orig) return;
...
function setUT(val, el) {
  el.closest('.ul-tabs').querySelectorAll('.ul-tab').forEach(function(t){t.classList.remove('active');});
  el.classList.add('active');
  var s = document.getElementById('userFilter');
  if (s) { s.value = val; loadUsers(); }
}
function setVT(val, el) {
  el.closest('.pill-tabs').querySelectorAll('.pill-tab').forEach(function(t){t.classList.remove('active');});
  el.classList.add('active');
  loadVerifications();
}
function setOffFilter(val, el) {
  el.closest('.pill-tabs').querySelectorAll('.pill-tab').forEach(function(t){t.classList.remove('active');});
  el.classList.add('active');
  var s = document.getElementById('offerStatusFilter');
...
function setRepFilter(val, el) {
  el.closest('.pill-tabs').querySelectorAll('.pill-tab').forEach(function(t){t.classList.remove('active');});
  el.classList.add('active');
  var s = document.getElementById('reportStatusFilter');
...
function setAnalPeriod(val, el) {
  el.closest('.seg').querySelectorAll('.seg-btn').forEach(function(t){t.classList.remove('on');});
  el.classList.add('on');
  var inp = document.getElementById('analyticsPeriod');
  if (inp) inp.value = val;
  loadAnalytics();
}
function exportCSV(type) {
  var token = localStorage.getItem('sb_token') || '';
  window.open('api/admin.php?action=export_csv&type=' + type + '&token=' + token, '_blank');
}

// ═══════════════════════════════════════════════════════════════════════
// DASHBOARD override – enrich after original loadDashboard runs
// ═══════════════════════════════════════════════════════════════════════
(function() {
  var orig = (typeof loadDashboard === 'function') ? loadDashboard : null;
  if (!orig) return;
  window.loadDashboard = async function() {
    await orig();
    // mirror stats to listings page mini-cards
    var al = (document.getElementById('statListings') || {}).textContent;
    var pl = (document.getElementById('statPending') || {}).textContent;
    if (al) _set('listStatActive', al);
    if (pl) _set('listStatPending', pl);
    // quick action sub text from nav badges
    var pv = document.getElementById('navBadgeVerif');
    var pr2 = document.getElementById('navBadgeReports');
...
async function _loadDashListings() {
  var tbody = document.getElementById('dashListingsBody');
  if (!tbody) return;
  try {
...
// Override chart renderers for new column-bar design
(function() {
  var orig = (typeof renderSignupsChart === 'function') ? renderSignupsChart : null;
  if (!orig) return;
  window.renderSignupsChart = function(data) {
    var el = document.getElementById('signupsChart');
    if (!el) return;
    if (!data || !data.length) { el.innerHTML = '<div class="empty-state" style="width:100%;padding:10px"><small>No data</small></div>'; return; }
    var max = Math.max.apply(null, data.map(function(d){return d.count;}));
    if (max < 1) max = 1;
    el.innerHTML = data.map(function(d) {
      var h = Math.max(Math.round((d.count/max)*100), 4);
      return '<div class="col-bar-wrap"><div class="col-bar" style="height:' + h + 'px;background:var(--pr);opacity:.85"></div></div>';
    }).join('');
  };
})();

(function() {
  var orig = (typeof renderCatsChart === 'function') ? renderCatsChart : null;
  if (!orig) return;
  window.renderCatsChart = function(data) {
    var el = document.getElementById('catsChart');
    if (!el) return;
    if (!data || !data.length) { el.innerHTML = '<div class="empty-state" style="padding:12px"><small>No data</small></div>'; return; }
...
window.loadUsers = async function() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = users.map(function(u) {
      var name = _esc(u.display_name||u.displayName||'—');
      var joined = _date(u.createdAt||u.created_at);
      var stDot = u.banned ? 'var(--red)' : u.verified ? 'var(--green)' : 'var(--yellow)';
      var stLabel = u.banned ? 'Banned' : u.verified ? 'Active' : 'Pending';
      var statusEl = '<span style="display:flex;align-items:center;gap:5px;color:' + stDot + ';font-weight:700"><span style="width:7px;height:7px;border-radius:50%;background:' + stDot + ';display:inline-block"></span>' + stLabel + '</span>';
...
window.loadListingsAdmin = async function() {
  ...
  try {
    ...
    if (d.stats) {
      _set('listStatTotal', d.stats.total); _set('listStatActive', d.stats.active);
      _set('listStatPending', d.stats.pending); _set('listStatExpired', d.stats.expired);
    } else {
      _set('listStatTotal', d.total||items.length);
      _set('listStatPending', items.filter(function(l){return l.status==='pending';}).length);
      _set('listStatActive', items.filter(function(l){return l.status==='active';}).length);
      _set('listStatExpired', items.filter(function(l){return l.status==='expired';}).length);
    }
    ...
    if (tbody) tbody.innerHTML = items.map(function(l) {
      ...
      var actions = l.status === 'pending'
        ? '<button class="act act-ok" onclick="approveListing(' + l.id + ')">Approve</button><button class="act act-no" onclick="rejectListingAdmin(' + l.id + ')">Reject</button>'
        : l.status === 'expired'
          ? '<button class="act act-view" onclick="approveListing(' + l.id + ')">Relist</button>'
          : '<button class="act act-ok" onclick="approveListing(' + l.id + ')">Approve</button><button class="act act-no" onclick="rejectListingAdmin(' + l.id + ')">Reject</button>';
...
window.loadPayments = async function() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = items.map(function(p, i) {
      ...
      var approveBlock = p.status === 'pending'
        ? '<div style="display:flex;gap:4px"><button class="act act-ok" onclick="approvePayment(' + p.id + ')">Approve</button><button class="act act-del" onclick="rejectPayment(' + p.id + ')">Reject</button></div>'
        : '<span class="badge ' + (sc[p.status]||'bc') + '" style="text-transform:capitalize">' + _esc(p.status) + '</span>';
      var _ssUrl = p.screenshotUrl || p.receiptUrl || p.screenshot_url || '';
      return '<tr><td style="font-family:var(--mono);font-size:12px;font-weight:700;color:var(--text2)">' + trxId + '</td><td><div style="display:flex;align-items:center;gap:8px"><div class="uav uav-sm">' + (p.userName||'?')[0].toUpperCase() + '</div><div><div style="font-weight:700;font-size:12.5px">' + _esc(p.userName||'—') + '</div><div style="font-size:10.5px;color:var(--text3)">' + _esc(p.userEmail||'') + '</div></div></div></td><td style="font-weight:800;font-size:14px">$' + (p.amount||0) + '</td><td style="color:var(--text2)">' + _date(p.createdAt||p.created_at) + '</td><td>' + approveBlock + '</td><td>' + (_ssUrl ? '<button class="act act-view" onclick="window.open('' + _ssUrl + '','_blank')">Receipt</button>' : '<span style="color:var(--text3)">—</span>') + '</td></tr>';
    }).join('');
...
// ═══════════════════════════════════════════════════════════════════════
(function() {
  var orig = (typeof loadVerifications === 'function') ? loadVerifications : null;
  if (!orig) return;
  window.loadVerifications = async function() {
    ...
    try {
      ...
      _set('verifPendingBadge', items.length||'0');
      _set('queueStatusCount', (items.length||0) + ' Pending Reviews');
      ...
      if (body) body.innerHTML = items.map(function(v) {
        ...
        var verifData = JSON.stringify({id:v.id, displayName:v.displayName||v.display_name, email:v.email, docs:v.docs||[]}).replace(/"/g,'&quot;');
        return '<div class="vr" data-verif="' + verifData + '" onclick="_selectVerif(this)"><div class="uav uav-sm">' + (name[0]||'?') + '</div><div class="vr-info"><div class="vr-name">' + name + '</div><div style="display:flex;align-items:center;gap:6px;margin-top:3px"><span style="font-size:11px;color:var(--text2)">' + _esc(docType) + '</span><span class="' + badgeCls + '">' + badgeLabel + '</span></div></div><div style="text-align:right;flex-shrink:0"><div class="vr-time">' + timeStr + '</div><div class="vr-score">ID Score: ' + score + '%</div></div></div>';
      }).join('');
...
// ═══════════════════════════════════════════════════════════════════════
// COUPONS  (API: 'get_coupons' – direct fetch)
// ═══════════════════════════════════════════════════════════════════════
window.loadCoupons = async function() {
  ...
  try {
    var token = localStorage.getItem('sb_token')||'';
    var r = await fetch('api/admin.php?action=get_coupons', {headers:{'Authorization':'Bearer '+token}});
    var d = await r.json();
    ...
    if (tbody) tbody.innerHTML = coupons.map(function(c) {
      var used = c.uses_count||0; var max = c.max_uses||0;
      var pct = max > 0 ? Math.min(Math.round((used/max)*100),100) : 0;
      var pctColor = pct>=100?'var(--red)':pct>=80?'var(--yellow)':'var(--pr)';
      var val = c.type==='percent' ? (c.value||c.discount_percent||0)+'% Off' : '$'+(c.value||c.discount_amount||0)+' Off';
      var exp = c.expires_at ? _date(c.expires_at) : 'No Expiry';
      return '<tr><td><div style="font-weight:900;color:var(--pr);font-family:var(--mono);font-size:13px">' + _esc(c.code||'—') + '</div><div style="font-size:11px;color:var(--text3)">Created ' + _date(c.created_at) + '</div></td><td><span style="font-weight:800;color:var(--pr)">' + val + '</span></td><td><div style="display:flex;align-items:center;gap:8px"><span style="font-size:12px;color:var(--text2);white-space:nowrap">' + used + '/' + (max||'Unlimited') + '</span><div style="flex:1;height:6px;background:var(--border);border-radius:3px;overflow:hidden;min-width:60px"><div style="width:' + pct + '%;height:100%;background:' + pctColor + ';border-radius:3px"></div></div><span style="font-size:11px;font-weight:800;color:var(--text2)">' + pct + '%</span></div></td><td style="font-size:12.5px;color:var(--text2)">' + exp + '</td><td><label class="toggle"><input type="checkbox"' + (c.is_active?' checked':'') + ' onchange="toggleCoupon(' + c.id + ')"><span class="toggle-slider"></span></label><span style="font-size:11.5px;font-weight:700;color:' + (c.is_active?'var(--green)':'var(--text3)') + ';margin-left:6px">' + (c.is_active?'Active':'Inactive') + '</span></td><td><div class="act-group"><button class="ib" onclick="openCouponModal(' + c.id + ')"><span class="ms">edit</span></button><button class="ib ib-del" onclick="deleteCoupon(' + c.id + ')"><span class="ms">delete</span></button></div></td></tr>';
    }).join('');
  } catch(e) { if(tbody) tbody.innerHTML = '<tr><td colspan="6" class="empty-row" style="color:var(--red)">' + _esc(e.message) + '</td></tr>'; }
};

// ═══════════════════════════════════════════════════════════════════════
// AFFILIATES  (API: 'get_affiliates' – direct fetch)
// ═══════════════════════════════════════════════════════════════════════
window.loadAffiliates = async function() {
  ...
  try {
    var token = localStorage.getItem('sb_token')||'';
    var r = await fetch('api/admin.php?action=get_affiliates', {headers:{'Authorization':'Bearer '+token}});
    var d = await r.json();
...
// ═══════════════════════════════════════════════════════════════════════
// MESSAGES  (API: 'list_conversations')
// ═══════════════════════════════════════════════════════════════════════
window.loadAdminMessages = async function() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = rows.map(function(r) {
      var p1=r.participant1||''; var p2=r.participant2||''; var cnt=r.message_count||0;
...
window.loadAnalytics = async function() {
  ...
  try {
    ...
    if (chart && a.daily && a.daily.length) {
      var max = Math.max.apply(null, a.daily.map(function(x){return x.count||0;}));
      if (max < 1) max = 1;
      chart.innerHTML = a.daily.map(function(x) {
        var h = Math.max(Math.round((x.count/max)*140), 4);
        var label = x.date ? x.date.substring(5) : '';
        return '<div class="col-bar-wrap"><div class="col-bar" style="height:'+h+'px;background:var(--pr);opacity:.8"></div><div class="col-label">'+label+'</div></div>';
      }).join('');
    }
...
window.loadRevenue = async function() {
  ...
  try {
    ...
    _set('revChurn', (r.churn_rate||'—')+'%'); _set('revAvg', _money(r.avg_amount||0));
...
window.loadAnnouncements = async function() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = rows.map(function(a) {
      var isActive=!!a.is_active; var isDraft=!isActive&&a.is_draft;
      var stColor=isActive?'var(--green)':isDraft?'var(--yellow)':'var(--text3)';
      var stLabel=isActive?'Active':isDraft?'Draft':'Inactive';
...
// ═══════════════════════════════════════════════════════════════════════
// PUSH NOTIFICATIONS  (API: 'push_stats')
// ═══════════════════════════════════════════════════════════════════════
...
window.loadCategories = async function() {
  ...
  try {
    ...
    _set('catActive', cats.reduce(function(s,c){return s+(c.active_count||c.count||0);},0).toLocaleString());
    ...
    var max = Math.max.apply(null, cats.map(function(c){return c.count||0;}));
    if (max < 1) max = 1;
    ...
    var catIcons = {Car:'directions_car',Electronics:'devices',Land:'terrain',House:'home',Jobs:'work',Fashion:'checkroom',Food:'restaurant',Vehicles:'directions_car'};
    if (tbody) tbody.innerHTML = cats.map(function(c, i) {
      var pct = Math.round((c.count||0)/max*100);
      var name = c.category||c.name||'Unknown';
      var iconKey = Object.keys(catIcons).find(function(k){return name.includes(k);});
      var icon = catIcons[iconKey]||'category';
      var color = colors[i%colors.length];
      return '<tr><td><div style="display:flex;align-items:center;gap:10px"><div style="width:36px;height:36px;border-radius:9px;background:'+color+'20;display:flex;align-items:center;justify-content:center;flex-shrink:0"><span class="ms" style="color:'+color+';font-size:18px">'+icon+'</span></div><span style="font-weight:700">'+_esc(name)+'</span></div></td><td style="font-weight:700">'+(c.count||0).toLocaleString()+'</td><td style="font-weight:700">'+(c.active_count||Math.floor((c.count||0)*.9)).toLocaleString()+'</td><td><div style="width:100px;height:6px;background:var(--border);border-radius:3px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:'+color+';border-radius:3px"></div></div></td><td><span class="badge '+(trendCls[i%trendCls.length])+'">'+(trends[i%trends.length])+'</span></td><td><button class="ib"><span class="ms">edit</span></button></td></tr>';
    }).join('');
  } catch(e) { if(tbody) tbody.innerHTML = '<tr><td colspan="6" class="empty-row" style="color:var(--red)">' + _esc(e.message) + '</td></tr>'; }
};
...
window.loadBlacklist = async function() {
  ...
  try {
    ...
    var rCls = function(reason) {
      var r=(reason||'').toLowerCase();
      if(r.includes('fraud')||r.includes('identity')) return 'bl-identity';
      if(r.includes('multiple')) return 'bl-multiple';
      if(r.includes('payment')) return 'bl-payment';
      return 'bl-policy';
    };
...
// ═══════════════════════════════════════════════════════════════════════
// SETTINGS  (API: 'stats' + 'log')
// ═══════════════════════════════════════════════════════════════════════
window.loadSettings = async function() {
  try {
    var d = await adminFetch('stats');
    var s = d.stats||d||{};
    _set('dbUsers', (s.total_users||0).toLocaleString()); _set('dbListings', (s.total_listings||0).toLocaleString());
    _set('dbMessages', (s.total_messages||0).toLocaleString()); _set('dbRevenue', '$'+(s.total_revenue||0));
    _set('dbOffers', (s.total_offers||0).toLocaleString()); _set('dbConvos', (s.total_conversations||0).toLocaleString());
  } catch(e) { console.warn('settings stats:', e); }
  try {
    var d2 = await adminFetch('log');
    var items = Array.isArray(d2) ? d2 : (d2.log||[]);
    var tbody = document.getElementById('dbStats');
    if (tbody) {
      ...
      tbody.innerHTML = items.slice(0,10).map(function(l) {
        return '<tr><td style="font-family:var(--mono);font-size:12px;color:var(--text2)">'+_esc(l.timestamp||l.created_at||'—')+'</td><td style="font-weight:600">'+_esc(l.action||'—')+'</td><td style="color:var(--pr);font-weight:700">'+_esc(l.admin_name||l.user||'SYSTEM')+'</td><td><span class="badge bg">Success</span></td></tr>';
      }).join('');
    }
  } catch(e) { console.warn('settings log:', e); }
};
...
window.loadLog = async function() {
  ...
  try {
    ...
    if (logBody) logBody.innerHTML = '<table class="tbl"><thead><tr><th>Timestamp</th><th>Action</th><th>Admin</th><th>Status</th></tr></thead><tbody>' +
