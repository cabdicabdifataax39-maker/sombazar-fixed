  ...
  async loadDropdown() {
    ...
    try {
      ...
      if (!d.success || !d.data.notifications.length) {
        wrap.innerHTML = '<div style="padding:20px;text-align:center;color:#94a3b8;font-size:13px">🔔 No notifications yet</div>';
        return;
      }
      const items = d.data.notifications.map(n => `
        <a href="${n.link || '#'}" onclick="Notif.markRead(${n.id})" style="display:flex;gap:10px;padding:10px 14px;text-decoration:none;color:inherit;border-bottom:1px solid #f1f5f9;background:${n.isRead?'white':'#f8faff'};transition:background .15s" onmouseenter="this.style.background='#f1f5f9'" onmouseleave="this.style.background='${n.isRead?'white':'#f8faff'}'">
          <div style="font-size:22px;flex-shrink:0;width:36px;text-align:center">${n.icon}</div>
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:10px;color:#94a3b8;margin-top:3px">${n.timeAgo}</div>
          </div>
          ${!n.isRead ? '<div style="width:8px;height:8px;background:#f97316;border-radius:50%;flex-shrink:0;margin-top:4px"></div>' : ''}
        </a>`).join('');
      wrap.innerHTML = items + `
        <div style="padding:10px;text-align:center">
          <button onclick="Notif.markAllRead()" style="background:none;border:none;color:#f97316;font-size:12px;font-weight:700;cursor:pointer">Mark all as read</button>
          <a href="profile.html?tab=notifications" style="color:#64748b;font-size:12px;margin-left:12px;text-decoration:none">See all →</a>
        </div>`;
    } catch(e) {
