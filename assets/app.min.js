...
function escHTML(str){const d=document.createElement('div');d.appendChild(document.createTextNode(str==null?'':String(str)));return d.innerHTML;}
const API_BASE = 'api';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (user && signinBtn && userArea) {
    ...
    if (avatar) {
      if (user.photoURL) {
        const _dn0=escHTML((user.displayName||'U')[0]);avatar.innerHTML=`<img src="${escHTML(user.photoURL)}" alt="${_dn0}" style="width:32px;height:32px;border-radius:50%;object-fit:cover;display:block" onerror="this.parentElement.textContent=this.alt">`;
      } else {
  ...
  document.addEventListener('auth:change', ({ detail }) => {
    ...
    if (u && signinBtn && userArea) {
      ...
      if (avatar) {
        if (u.photoURL) {
          const _u0=escHTML((u.displayName||'U')[0]);avatar.innerHTML=`<img src="${escHTML(u.photoURL)}" alt="${_u0}" style="width:32px;height:32px;border-radius:50%;object-fit:cover;display:block" onerror="this.parentElement.textContent=this.alt">`;
        } else {
...
function toggleNotifDropdown() {
  ...
  if (!isOpen) {
    ...
    try { Notif.loadDropdown(); } catch(e) {
      dd.innerHTML = '<div style="padding:20px;text-align:center;color:#94a3b8;font-size:13px;">Loading...</div>';
      fetch('api/notifications.php?action=list&limit=10', {
        headers: { Authorization: 'Bearer ' + (localStorage.getItem('sb_token') || '') }
      }).then(r=>r.json()).then(d=>{
        if (!d.success || !d.data.notifications.length) {
          dd.innerHTML = '<div style="padding:24px;text-align:center;color:#94a3b8;font-size:13px;">No notifications</div>';
          return;
        }
        dd.innerHTML = '<div style="padding:12px 16px;font-weight:800;font-size:14px;border-bottom:1px solid #f1f5f9;">Notifications</div>' +
          d.data.notifications.slice(0,10).map(n=>
            '<a href="' + (n.link||'#') + '" style="display:flex;gap:10px;padding:12px 16px;border-bottom:1px solid #f1f5f9;text-decoration:none;color:inherit;">' +
            '<div style="flex:1;"><div style="font-size:13px;font-weight:' + (n.isRead?'500':'700') + ';color:#0f172a;">' + n.title + '</div>' +
            (n.body ? '<div style="font-size:11px;color:#64748b;">' + n.body + '</div>' : '') +
            '<div style="font-size:10px;color:#94a3b8;margin-top:2px;">' + (n.timeAgo||'')+'</div></div>' +
            (!n.isRead ? '<div style="width:7px;height:7px;background:#ec5b13;border-radius:50%;margin-top:4px;flex-shrink:0;"></div>' : '') +
            '</a>'
          ).join('') +
          '<div style="padding:10px;text-align:center;"><a href="notifications.html" style="font-size:12px;color:#ec5b13;font-weight:700;text-decoration:none;">View all</a></div>';
      }).catch(()=>{
        dd.innerHTML = '<div style="padding:20px;text-align:center;color:#94a3b8;font-size:13px;">Could not load</div>';
      });
    }
